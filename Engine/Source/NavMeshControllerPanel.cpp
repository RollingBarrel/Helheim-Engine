...
void NavMeshControllerPanel::DrawDebug()
{
    ...
    App->GetDebugDraw()->DrawSphere(&App->GetNavigation()->GetQueryResult()[0], &color[0], 1.0f);
    float3 color2 = float3(1.0f, 1.0f, 0.0f);
    App->GetDebugDraw()->DrawSphere(&App->GetNavigation()->GetQueryCenter()[0], &color2[0], 1.0f);
