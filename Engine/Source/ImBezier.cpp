...
namespace ImGui
{
    ...
    float CurveValueSmooth(float p, const std::vector<float2>& points)
    {
        if (points.size() < 2 || points.empty())
            return 0;
        if (p < 0) return points[0].y;

        float* input = new float[points.size() * 2];
        float output[4];
        ...
        spline(input, points.size(), 1, p, output);

        delete[] input;
        return output[0];
    }

    float CurveValue(float p, const std::vector<float2>& points)
    {
        if (points.size() < 2 || points.empty())
            return 0;
        if (p < 0) return points[0].y;
        ...
        int left = 0;
        while (left < points.size() && points[left].x < p && points[left].x != -1) left++;
        if (left) left--;
    ...
    int Curve(const char* label, const ImVec2& size, const int maxpoints, std::vector<float2>& points)
    {
        int modified = 0;
        int i;
        if (points.size() < 2)
        ...
        ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size.x,size.y));
        ItemSize(bb);
        if (!ItemAdd(bb, NULL))
            return 0;
        ...
        do
        {
            ...
            {
                modified = 1;
                points.erase(points.begin() + kill);
                max--;
                kill = 0;
            }
        ...
        if (hovered)
        {
            ...
            if (g.IO.MouseDown[0])
            {
                ...
                {
                    points.insert(points.begin() + left +1, pos);
                    ++max;
                }
