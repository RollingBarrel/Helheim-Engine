  ...
  _height(0)
{
    ...
    _projection = float4x4::D3DOrthoProjLH(0.1f, 100.0f, 2.f, 2.f);  // View: left, right, bottom, top (we are using normalized coords)

    _view = float4x4::LookAt(float3(0, 0, 1),  // Camera position in world space (doesn't really apply when using an ortho projection matrix)
                        float3(0, 0, 0),  // look at origin
                        float3(0, 1, 0), // Head is up (set to 0, -1, 0 to look upside down)
                        float3(0,1,0)); 

    _model = float4x4::identity; // Identity matrix

    recalculateMVP();

    // Load the shaders
    _programId = glCreateProgram();
    GLUtils::loadShader("Assets\\Shaders\\fontVertex.shader", GL_VERTEX_SHADER, _programId);
    GLUtils::loadShader("Assets\\Shaders\\fontFragment.shader", GL_FRAGMENT_SHADER, _programId);
    ...
    glUniform4fv(_uniformTextColorHandle, 1, _textColor.ptr());
    glUniformMatrix4fv(_uniformMVPHandle, 1, GL_FALSE, _mvp.ptr());
...
void FTLabel::rotate(float degrees, float x, float y, float z) {
    ...
    _model = float4x4::RotateAxisAngle( float3(x, y, z), rad);
    recalculateMVP();
}

void FTLabel::scale(float x, float y, float z) {
    _model = float4x4::Scale(float3(x, y, z)) * _model;
    recalculateMVP();
}
