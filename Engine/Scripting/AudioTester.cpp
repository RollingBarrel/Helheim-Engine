...
void AudioTester::ControlSFX()
{
    ...
    if (input->GetKey(Keys::Keys_4) == KeyState::KEY_DOWN) 
    {
        if (!mSFX4Play) 
        {
            if (mSFX4sID == -1)
            {
                mSFX4sID = GameManager::GetInstance()->GetAudio()->Play(SFX::PLAYER_GUNFIRE, mSFX4sID, mGameObject->GetWorldPosition());
            }
            else 
            {
                GameManager::GetInstance()->GetAudio()->Play(SFX::PLAYER_GUNFIRE, mSFX4sID, mGameObject->GetWorldPosition());
            }
            mSFX4Play = true;
        }
        else
        {
            mSFX4sID = GameManager::GetInstance()->GetAudio()->Release(SFX::PLAYER_GUNFIRE, mSFX4sID);
            mSFX4Play = false;
        }
    }
}
