...
void HudController::Start()
{
    ...
    if (mLoseScreen)
    {
        ...
        if (mLoseMenuBtnGO)
        {
            mLoseBtn = static_cast<ButtonComponent*>(mLoseMenuBtnGO->GetComponent(ComponentType::BUTTON));
        ...
        if (mTryAgainBtnGO) {
            mTryAgainBtn = static_cast<ButtonComponent*>(mTryAgainBtnGO->GetComponent(ComponentType::BUTTON));
